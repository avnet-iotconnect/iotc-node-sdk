...
var dc = "<<Your Connection String>>";
var sIdd = "<<Your SID>>";

async.series([
    function(cb_series) {
        rl.question('Enter device connection sting : ', function (deviceConnectionString) {
            deviceConnectionString = dc;
            rl.question('Enter SID : ', function (sId) {
                ...
                if(sdkOptions.certificate){
                            ...
                            if(CERT_PATH_FLAG == true) {
                                try {
                                    if(!isDeviceConnected) {
                                        iotConnectSDK = new SDKClient(deviceConnectionString, sId, deviceCallback, twinUpdateCallback, sdkOptions, env);
                                    } else {
                                ...
                                console.log("Set proper certificate file path and try again.")
                                process.exit();
                            }
                        });
                    } catch (error) {
                    ...
                    if(!isDeviceConnected) {
                        ...
                        iotConnectSDK = new SDKClient(deviceConnectionString, sId, sdkOptions, function(response){
                            // console.log("Sdk callback => ", response);
                            if(response.status){
