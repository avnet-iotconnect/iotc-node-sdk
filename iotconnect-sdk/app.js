...
class Init {
  constructor(cpId, uniqueId, messageCallback, twinCallback, sdkOptions = "", env = "") {
    this.commonLib = new CommonFunctions(cpId, uniqueId);
	...
	if(sdkOptions && 'devicePrimaryKey' in sdkOptions) {
		this.DEVICE_PRIMARY_KEY = sdkOptions.devicePrimaryKey;
    } else {
              ...
              if(hbStatusFlag == 0)
              {
                ...
                if((response.data.at == config.authType.CA_SIGNED || response.data.at == config.authType.CA_SELF_SIGNED) && self.CERT_PATH_FLAG == false){
                  ...
                  // process.exit();
					...
					if(response.data.at == config.authType.SYMMETRIC_KEY ) {
						var deviceSyncRes = cache.get(self.CPID+"_"+uniqueId);
						var brokerConfiguration = deviceSyncRes.p;
						self.generateSasToken(brokerConfiguration.h, self.DEVICE_PRIMARY_KEY, null, config.sasTokenExpiryTime, function (sasToken) {
							brokerConfiguration['pwd'] = sasToken;
  ...
  deviceConnectionProcess(callback) {
    ...
    self.clientConnection(function (clientResponse) {
      if (clientResponse.status) {
        ...
        self.startCommandSubscriber(self.CPID, self.UNIQUE_ID, clientResponse.data, function (response) {
  ...
  Date   : 2018-01-24
  */
  startCommandSubscriber(cpId, uniqueId, clientData, callback) {
	var self = this;
    try {
      this.commonLib.subscriberProcess(cpId, uniqueId, clientData, this.DEVICE_CONNECTED, this.offlineFileConfig, this.IS_DEBUG, function (response) {
        if (response.status) {
  ...
  */
  manageCommand(cpId, uniqueId, response, callback) {
	...
	case config.commandType.DATA_FREQUENCY_UPDATE: //17 - Data Frequency Updated
        self.commonLib.manageDebugLog("INFO_CM11", self.UNIQUE_ID, self.CPID, "", 1, self.IS_DEBUG);
		var deviceSyncResDf = cache.get(self.CPID+"_"+self.UNIQUE_ID);
		deviceSyncResDf.sc.df = response.data.data.df;
		cache.put(self.CPID+"_"+self.UNIQUE_ID, deviceSyncResDf);
		var requestedParams = undefined;
        break;
  ...
  startEdgeDeviceProcess(requestedParams="") {
    ...
    if (responseData.ee == config.edgeEnableStatus.enabled) {
      async.series([
          ...
          try {
            self.commonLib.setEdgeConfiguration(responseData.att, self.UNIQUE_ID, responseData.d, function (res) {
              if(res.status){
                ...
                // console.log("config.commandType.ATTRIBUTE_INFO_UPDATE ==> ",config.commandType.ATTRIBUTE_INFO_UPDATE);
                // return false;
        ...
        // console.log("Edge device Data managed.")
      })
    }
  }
  ...
  */
  updateTwin(key, value) {
	var self = this;
    try {
      if (this.STOP_SDK_CONNECTION == true && key &&  (value || value === null)) {
        var obj = {};
        obj[key] = value;
        obj['cpId'] = this.CPID;
        this.commonLib.UpdateTwin(obj, this.UNIQUE_ID, this.CLIENT_CONNECTION, this.offlineFileConfig, this.IS_DEBUG, function (response) {
          if(response.status) {
  ...
  */
  dispose(sdkconnection="") {
    try {
      if (this.CLIENT_CONNECTION && this.DEVICE_CONNECTED) {
        var deviceConnection = this.CLIENT_CONNECTION;
        deviceConnection.end();
        this.DEVICE_CONNECTED = false;
        this.STOP_SDK_CONNECTION = false;
        if(sdkconnection != ""){
          this.commonLib.deleteAllLogFile(this.LOG_PATH);
        } 
      } else {
...
/* 
  ...
  getAllTwins() {
    ...
    try {
      if (this.STOP_SDK_CONNECTION == true && this.DEVICE_CONNECTED == true) {
        this.commonLib.getAllTwins(function (response) {
  ...
  generateSasToken(resourceUri, signingKey, policyName, expiresInMins, callback) {
